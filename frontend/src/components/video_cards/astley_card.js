    ...
    <CustomCard inView={inView}>
      ...
      <div ref={inViewRef} style={{ position: 'relative', paddingTop: '56.25%' }}>
        <ReactPlayer
          controls={false}
          ref={playerRef}
          playing={canPlay && Object.keys(videosInView).some((video) => videosInView[video])}
      ...
      {canPlay ? (
        ...
        <Button style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }} onClick={() => setCanPlay(true)}>
          Can we autoplay this video?
        </Button>
      )}
    </CustomCard>
  );
