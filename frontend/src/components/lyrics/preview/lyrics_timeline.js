...
export default function LyricsTimeLine({videoDuration, syncedLyrics, changeLyricById, aboveProgressBar}) {
    ...
    syncedLyrics.forEach((line) => {
      line.forEach((syncedLyric) => {
        ...
        const filterByParity = (id) => {
          return aboveProgressBar ? id % 2 === 0 : id % 2 === 1
        }
        ...
        if ((-5 < diff && diff < 5 || !syncedLyric.time) && filterByParity(syncedLyric.id)) {
          ...
          tempMappedLyrics[Math.floor(syncedLyric.id / 2) % 3].push(syncedLyric)
        }
      })
    })
        ...
        return (
          <Row key={index} style={{marginTop: '8%'}} >
            {row.map((syncedLyric) => {
              ...
              const timePixelOffset = width * 0.8 * timelineRatio
