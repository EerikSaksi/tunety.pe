  ...
  return (
    ...
    ?
      <Suspense fallback={<Loading centered />}>
        <LyricsSyncCreator startTime={leftPixelOffset / (0.8 * width) * data.youtubeVideoData.duration} endTime={(0.8 * width - rightPixelOffset) / (0.8 * width) * data.youtubeVideoData.duration} />
      </Suspense>
      :
      <Container ref={rootRef} fluid style={{paddingLeft: 0, paddingRight: 0}}>
        ...
        >
          <Button onDrag={(event => console.log(event))} style={{position: 'absolute', width: `calc(80% - ${leftPixelOffset + rightPixelOffset}px)`, height: 80, left: `calc(10% + ${leftPixelOffset}px})`, right: `calc(10% + ${rightPixelOffset}px)`, bottom: 60, }}>
          </Button>
        </DraggableCore>
        <div style={{position: 'absolute', transition: 'all linear 40ms', left: `${(videoDuration / data.youtubeVideoData.duration) * 80 + 10}%`, height: 120, bottom: 60, }}>
          <DraggableCore axis='x'
            onStart={() => {
              setDraggingPin(true)
            }}
            ...
            onDrag={(event) => {
              ...
              const movementTime = data.youtubeVideoData.duration * movementRatio

              setVideoDuration(videoDuration => {
                const newTime = videoDuration + movementTime
                //in range
                if (0 < newTime && newTime < data.youtubeVideoData.duration) {
                  return newTime
                }
                else {
          ...
          >
            <Button variant="danger" style={{position: 'absolute', height: 20, width: 20, bottom: 80, transform: 'translate(-10px, 0px)'}} >
            </Button>
          </DraggableCore>
          <div style={{position: 'absolute', width: 3, height: 80, backgroundColor: 'black', zIndex: 1000, bottom: 0}}>
          </div>
        </div>
      </Container>
  )
